    ...
    <div className="grid grid-cols-2 divide-x divide-y divide-line overflow-hidden rounded-lg border border-line bg-panel sm:grid-cols-4 sm:divide-y-0">
      {items.map((item) => (
        <div key={item.label} className="px-4 py-4 sm:px-5 sm:py-5">
          <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-textMuted">{item.label}</p>
          <p className="mt-1.5 font-heading text-xl font-semibold text-textPrimary sm:text-2xl">
